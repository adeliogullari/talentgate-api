...
async def test_retrieve_countries(client: TestClient) -> None:
    response = client.get(url="/api/v1/location/countries")
...
async def test_retrieve_states_by_country(client: TestClient) -> None:
    response = client.get(url="/api/v1/location/countries/Germany/states")
...
async def test_retrieve_cities_by_state(client: TestClient) -> None:
    response = client.get(
        url="/api/v1/location/countries/Germany/states/Bavaria/cities"
    )
